            ...
            <div>
              <el-table :data="reactiveFileData" row-key="id" header-row-class-name="thead-light" ref="fileTable">
                ...
                <el-table-column align="right" label="File">
                  <div slot-scope="{$index, row}" class="d-flex">
                    <base-button class="btn-tooltip" type="danger" size="sm" icon>
                      <a :href="row.path" download><i class="text-white fa fa-file-pdf"></i></a>
                    </base-button>
                  </div>
                </el-table-column>
...
export default {
  ...
  methods: {
    async fetchData( ) {
      ...
      await this.$store.dispatch(__.GETALL,'stato_ordine')
      await this.$store.dispatch(__.GETALL,'riga_ordine')
      await this.$store.dispatch(__.GETWHERE,{model:'file', cond:[{field:'tipo', op:'=', value:'ordine'}]})
      ...
      .map( f => {
        switch(f){
          case 'data_consegna':
            return {
              formatter: (row, column) => row[column.property] ? moment(row[column.property]).format('YYYY-MM-DD') : 'TBD',
              prop: f, 
              sortable: true,
              label: f.replace(/^\w/, c => c.toUpperCase())
            }
      ...
      this.tableData = this.$store.state.orders.records.map( record => {
        ...
        .reduce((cur, key) => {
          ...
          switch(key) {
            case 'progetto':
              ...
              break
            case 'stato':
              val =  (this.$store.state.orderStatus.records.filter( i => i.id == record[key] ).pop() || {}).name
              break
            default:
              val = record[key]
              break
        ...
        // row.children = this.$store.state.orderRows.records.filter( or => or.ordine == row.id)
        return row
        }
      )
